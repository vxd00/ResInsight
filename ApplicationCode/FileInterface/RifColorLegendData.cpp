...
cvf::ref<RigFormationNames> RifColorLegendData::readLyrFormationNameFile( const QString& fileName, QString* errorMessage )
{
    ...
    while ( !stream.atEnd() )
    {
        ...
        if ( lineSegs.size() == 3 ) // Normal case
        {
            ...
            QString colorWord = numberString.split(" ", QString::SkipEmptyParts).last();   // extract last word which may contain formation color

            if (QColor::isValidColor(colorWord)) numberString.remove(colorWord);           // remove color if present as last word on line

            QStringList numberWords = numberString.split( QRegExp( "-" ), QString::SkipEmptyParts );   // extract words containing formation number(s)
